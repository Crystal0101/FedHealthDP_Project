...
def add_dp_noise(gradients, sensitivity, epsilon, dynamic_factor):
    noise_scale = sensitivity / (epsilon * dynamic_factor)
    noise = np.random.laplace(0, noise_scale, size=gradients.shape).astype(np.float32)  
    
    if isinstance(gradients, np.ndarray):
        gradients = torch.tensor(gradients, dtype=torch.float32)  
        
    noise_tensor = torch.tensor(noise, dtype=gradients.dtype).to(gradients.device)  
    return gradients + noise_tensor
...
class FederatedLearningClient:
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            # 动态调整噪声因子
            self.dp_dynamic_factor = 1 + epoch * 0.1  # 每轮增加10%的因子作为示例
            ...
            for inputs, targets in train_loader:
                ...
                for param in self.model.parameters():
                    if param.grad is not None:
                        param.grad = add_dp_noise(param.grad, self.dp_sensitivity, self.dp_epsilon, self.dp_dynamic_factor)
...
class FederatedLearningServer:
    ...
    def aggregate_updates(self, client_updates):
        ...
        for k in client_updates[0].keys():
            ...
            if not isinstance(aggregated_update[k], torch.Tensor):
                aggregated_update[k] = torch.tensor(aggregated_update[k], dtype=torch.float32)  # 确保 aggregated_update 是 torch 张量
            aggregated_update[k] = add_dp_noise(aggregated_update[k], self.dp_sensitivity, self.dp_epsilon, self.dp_dynamic_factor)
