...
def merge_data_in_chunks(merged_data_path, chunk_generator, on_cols, suffix, left_on=None, right_on=None):
    ...
    for chunk in chunk_generator:
        ...
        merged_data = pd.read_csv(merged_data_path, low_memory=False)
...
def preprocess():
    try:
        ...
        # 加载数据
        admissions_chunks = load_data("fixed_ADMISSIONS.csv", usecols=admissions_cols, dtype=dtype_spec)
        patients_chunks = load_data("fixed_PATIENTS.csv", usecols=patients_cols, dtype=dtype_spec)
        labevents_chunks = load_data("fixed_LABEVENTS.csv", usecols=labevents_cols, dtype=dtype_spec)
        diagnoses_icd_chunks = load_data("fixed_DIAGNOSES_ICD.csv", usecols=diagnoses_icd_cols, dtype=dtype_spec)
        prescriptions_chunks = load_data("fixed_PRESCRIPTIONS.csv", usecols=prescriptions_cols, dtype=dtype_spec)
        procedures_icd_chunks = load_data("fixed_PROCEDURES_ICD.csv", usecols=procedures_icd_cols, dtype=dtype_spec)
        chartevents_chunks = load_data("fixed_CHARTEVENTS.csv", usecols=chartevents_cols, dtype=dtype_spec)
